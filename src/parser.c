...
{
    // the every third parameters is default parameter for every element
...
network parse_network_cfg(char *filename)
{
    ...
    // firstly, insert first parse into net which is [net] information
    ...
    while(n){
        ...
        n = n->next;
        ++count;
        // params like to be the previous output informations
        if(n){
            params.h = l.out_h;
            params.w = l.out_w;
            params.c = l.out_c;
    ...
    while((line=fgetl(file)) != 0){
        ...
        switch(line[0]){
            ...
            case ';':
                free(line);
                break;
            default: // give the line's value from file parse the value into curren->options
...
void load_weights_upto(network *net, char *filename, int cutoff)
{
    ...
    fflush(stdout);
    // weight is saved as binary file
    ...
    int minor;
    int revision;
    // As we can see the first 4 value is weight config value
    // read 4 int from file
